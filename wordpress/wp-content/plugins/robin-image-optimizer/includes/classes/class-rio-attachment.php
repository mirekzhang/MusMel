...
class WIO_Attachment {
	...
	public function optimize( $optimization_level = '' ) {
		...
		if ( ! file_exists( $path ) ) {
			...
			WIO_Logger::error( sprintf( 'Failed to find original attachment #%s located in %s. Skipping optimization. This may be caused due to bug in %s function, which returns false for attachment meta', $this->id, empty( $path ) ? '*empty path*' : $path, 'wp_get_attachment_metadata()' ) );
		...
		if ( is_wp_error( $is_image_backuped ) ) {
			...
			WIO_Logger::error( sprintf( 'Failed to make backup of original attachment #%s. Skipping optimization.', $this->id ) );
		...
		if ( is_wp_error( $optimized_img_data ) ) {
			...
			WIO_Logger::error( sprintf( 'Failed to process (url: %s, path: %s, quality: %s) as error was returned: %s', $this->get( 'url' ), $this->get( 'path' ), $image_processor->quality( $optimization_level ), $error_msg ) );
		...
		if ( $image_downloaded ) {
			...
			if ( function_exists( 'wp_get_image_mime' ) ) {
				...
				WIO_Logger::error( 'App is missing wp_get_image_mime() function, unable to get MIME type' );
			}
	...
	public function replaceOriginalFile( $optimized_img_data, $image_size = '' ) {
		...
		if ( empty( $optimized_file ) ) {
			WIO_Logger::error( sprintf( 'Unable to replace original image with new as failed to download %s', $optimized_img_url ) );
		...
		if ( $bytes === false ) {
			WIO_Logger::error( sprintf( 'Failed to put new image\'s %s content to %s as file_put_contents() failed', $optimized_img_url, $attachment_size_path ) );
	...
	protected function remoteDownloadImage( $url ) {
		...
		if ( ! function_exists( 'curl_version' ) ) {
			...
			if ( $content === false ) {
				WIO_Logger::error( sprintf( 'Failed to get content of "%s" using file_get_contents()', $url ) );
		...
		if ( $image_body === false ) {
			WIO_Logger::error( sprintf( 'Failed to get content of "%s" using curl_exec()', $url ) );
	...
	public function getImageSizeUrl( $size = 'thumbnail' ) {
		...
		$url  = str_replace( wrio_basename( $this->url ), $file, $this->url );
	...
	public function getImageSizePath( $size = 'thumbnail' ) {
		...
		$file = $this->attachment_meta['sizes'][ $size ]['file'];
		$path = str_replace( wrio_basename( $this->path ), $file, $this->path );
	...
	protected function resize() {
		...
		if ( is_wp_error( $image ) ) {
			WIO_Logger::error( sprintf( 'Failed to get image edit via wp_get_image_editor(), error: "%s"', $image->get_error_message() ) );

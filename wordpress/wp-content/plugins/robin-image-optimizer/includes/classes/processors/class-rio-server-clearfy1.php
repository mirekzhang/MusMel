...
class WIO_Image_Processor_Clearfy1 extends WIO_Image_Processor_Abstract {
	...
	public function process( $settings ) {
		...
		WIO_Logger::info( sprintf( "Preparing to upload a file (%s) to a remote server (%s).", $settings['image_path'], $this->server_name ) );

		// todo: need to use wp_remote*, see https://webcraftic.atlassian.net/browse/RIO-71
		$filename = $settings['image_path'];

		if ( ! class_exists( 'finfo' ) ) {
			WIO_Logger::error( 'For Robin image optimizer to work, you need to install php extension [php_fileinfo].' );
		...
		if ( $r["http_code"] != 200 ) {
			WIO_Logger::error( sprintf( 'Failed to get content of URL: %s as wp_remote_request() responded Http error (%s).', $upload_url, $r["http_code"] ) );
		...
		WIO_Logger::info( sprintf( "File successfully uploaded to remote server (%s).", $this->server_name ) );

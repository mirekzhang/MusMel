...
abstract class WIO_Image_Processor_Abstract {
	...
	protected function request( $type, $url, $body = null, array $headers = [] ) {
		...
		if ( is_wp_error( $response ) ) {
			WIO_Logger::error( sprintf( '%s returned error (%s).', $error_message, $response->get_error_message() ) );
		...
		if ( $response_code !== 200 ) {
			WIO_Logger::error( sprintf( '%s responded Http error (%s).', $error_message, $response_code ) );

			return new WP_Error( 'http_request_failed', sprintf( "Server responded an HTTP error %s", $response_code ) );
		}

		if ( empty( $response_body ) ) {
			WIO_Logger::error( sprintf( '%s responded an empty request body.', $error_message ) );

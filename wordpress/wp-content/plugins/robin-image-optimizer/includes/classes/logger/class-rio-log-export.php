...
class WIO_Log_Export {
	...
	public function prepare () {
		...
		if ( ! class_exists( '\ZipArchive' ) ) {
			WIO_Logger::error( 'App does not have \ZipArchive class available. It is not possible to prepare export' );
		...
		$log_base_dir = WIO_Logger::get_base_dir();

		if ( $log_base_dir === false ) {
			WIO_Logger::error( sprintf( 'Failed to get log path %s', $log_base_dir ) );
		...
		if ( isset( $uploads['error'] ) && $uploads['error'] !== false ) {
			WIO_Logger::error( 'Unable to get save path of ZIP archive from wp_get_upload_dir()' );
		...
		if ( ! $zip->open( $zip_save_path, ZipArchive::CREATE ) ) {
			WIO_Logger::error( sprintf( 'Failed to created ZIP archive in path %s. Skipping export...', $zip_save_path ) );
		...
		if ( ! empty( $log_files ) ) {
			foreach ( $log_files as $file ) {
				if ( ! $zip->addFile( $file, wp_basename( $file ) ) ) {
					WIO_Logger::error( sprintf( 'Failed to add %s to %s archive. Skipping it.', $file, $zip_save_path ) );
		...
		if ( ! empty( $system_info ) ) {
			...
			if ( false !== @file_put_contents( $system_info_path, $system_info ) ) {
				if ( ! $zip->addFile( $system_info_path, $system_info_file_name ) ) {
					WIO_Logger::error( sprintf( 'Failed to add %s to %s archive. Skipping it.', $system_info_file_name, $system_info_path ) );
				}
			} else {
				WIO_Logger::error( sprintf( 'Failed to save %s in %s', $system_info_file_name, $zip_save_path ) );
			}
		}

		if ( ! $zip->close() ) {
			WIO_Logger::error( sprintf( 'Failed to close ZIP archive %s for unknown reason. ZipArchive::close() failed.' ) );
		}
	...
	public function download ( $should_clean_up = true ) {
		...
		if ( $zip_save_path === false ) {
			WIO_Logger::error( sprintf( 'Failed to get ZIP %s content as file_get_contents() returned false', $zip_save_path ) );

...
class RIO_Process_Queue extends RIO_Base_Active_Record {
	...
	public static function count_by_status( $status ) {
		...
		$prepared_sql = $wpdb->prepare( $sql, array( $status ) );
	...
	public static function count_by_type_status( $type, $status ) {
		...
		$prepared_sql = $wpdb->prepare( $sql, array( $type, $status ) );
	...
	public static function get_levels() {
		return array( self::LEVEL_NORMAL, self::LEVEL_AGGRESIVE, self::LEVEL_CUSTOM, self::LEVEL_ULTRA );
	}
	...
	public function load() {
		global $wpdb;
		$table_name = static::table_name();
		$sql        = $wpdb->prepare( "SELECT * FROM {$table_name} WHERE object_id = %d AND item_type = %s LIMIT 1;", array(
			$this->object_id,
			$this->item_type,
		) );
	...
	public function save() {
		...
		if ( $this->id ) {
			...
			WIO_Logger::debug( sprintf( 'Updated queue item #%s, attributes values: %s', $this->id, wp_json_encode( $data ) ) );
			...
			if ( $count !== false && $count > 0 ) {
				...
				if ( ! empty( $lastId ) ) {
					...
					WIO_Logger::debug( sprintf( 'New queue created #%s, attributes values: %s', $this->id, wp_json_encode( $data ) ) );
				}
			}
		}
	...
	public function delete() {
		...
		$rows_affected = $wpdb->delete( $db_table, array( 'id' => $this->id ) );
		
		WIO_Logger::debug( sprintf( 'Deleted queue item #%s', $this->id ) );

...
class WIO_Backup {
		...
		if ( $this->isBackupWritable() ) {
			...
			if ( is_file( $file_path ) ) {
				if ( ! @copy( $file_path, $temp_file_path ) ) {
					WIO_Logger::error( sprintf( 'Failed to swap original file %s with %s as copy() failed.', $temp_file_path, $file_path ) );
			...
			WIO_Logger::info( sprintf( 'Creation of temporary attachment (%s) successfully completed!', $file_path ) );
	...
	protected function backupAttachmentSize( WIO_Attachment $wio_attachment, $image_size = '' ) {
		...
		if ( is_wp_error( $backup_dir ) ) {
			WIO_Logger::error( sprintf( 'Failed to create backup dir, error: %s', $backup_dir->get_error_message() ) );
		...
		$backup_file = $backup_dir . wrio_basename( $original_file );

		if ( is_file( $original_file ) ) {
			if ( ! @copy( $original_file, $backup_file ) ) {
				WIO_Logger::error( sprintf( 'Failed to copy %s to %s as copy() failed', $original_file, $backup_file ) );
			}
		}
	...
	protected function restoreAttachmentSize( WIO_Attachment $wio_attachment, $image_size = null ) {
		...
		if ( ! is_file( $backup_file ) ) {
			$error_msg = 'Unable to restore from a backup. There is no file.';

			WIO_Logger::error( sprintf( '%s, attachment id: %s, backup file: %s', $error_msg, $wio_attachment->get( 'id' ), $backup_file ) );

			return new WP_Error( 'cannot_restore_file', $error_msg );
		}

		if ( ! @copy( $backup_file, $original_file ) ) {
			WIO_Logger::error( sprintf( 'Failed to swap %s with %s as copy() failed', $backup_file, $original_file ) );

			return false;
		}

		// удаляем файл из бекапа
		if ( ! @unlink( $backup_file ) ) {
			WIO_Logger::error( sprintf( 'Failed to delete backup file %s as unlink() failed', $backup_file ) );
		}

		WIO_Logger::info( sprintf( 'Restored file: %s.', $backup_file ) );
	...
	public function removeAttachmentBackup( $attachment_id ) {
		...
		$main_file_path = $backup_dir . wrio_basename( $attachment_meta['file'] );

		if ( ! file_exists( $main_file_path ) ) {
			WIO_Logger::error( sprintf( "Failed to remove an attachment file. File (%s) isn't exists. Attachment #%s", $main_file_path, $attachment_id ) );
		}

		if ( ! @unlink( $main_file_path ) ) {
			WIO_Logger::error( sprintf( 'Failed to unlink a main file (%s) for attachment #%s', $main_file_path, $attachment_id ) );
		}
		...
		if ( isset( $attachment_meta['sizes'] ) && is_array( $attachment_meta['sizes'] ) ) {
			foreach ( $attachment_meta['sizes'] as $size ) {
				...
				if ( ! file_exists( $thumbnail_file_path ) ) {
					WIO_Logger::error( sprintf( "Failed to remove a thumbnail file. File (%s) isn't exists. Attachment #%s", $thumbnail_file_path, $attachment_id ) );
				}

				if ( ! @unlink( $thumbnail_file_path ) ) {
					WIO_Logger::error( sprintf( 'Failed to unlink thumbnail (%s) for attachment #%s', $thumbnail_file_path, $attachment_id ) );
				}
			}
		}
	...
	protected function mkdir( $dir ) {
		WIO_Logger::info( sprintf( 'Try to create backup directory. Backup dir: (%s)', $dir ) );

		if ( ! wp_mkdir_p( $dir ) ) {
			WIO_Logger::error( sprintf( 'Unable to create backup directory (%s) as mkdir() failed', $dir ) );
	...
	protected function restore_file( $backup_file, $original_file ) {
		if ( is_file( $backup_file ) ) {
			if ( ! @copy( $backup_file, $original_file ) ) {
				WIO_Logger::error( sprintf( 'Failed to swap original file (%s) with %s as copy() failed', $backup_file, $original_file ) );
		...
		WIO_Logger::error( sprintf( 'Unable to restore from a backup. There is no file (%s).', $backup_file ) );

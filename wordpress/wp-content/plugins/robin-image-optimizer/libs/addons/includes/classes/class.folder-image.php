...
class WRIO_Folder_Image {
	...
	public function createOptimizationData() {
		return new RIO_Process_Queue( array(
			'id'        => $this->id,
			'item_type' => 'cf_image',
		) );
	}
	...
	protected function loadOptimizationData() {
		...
		$sql        = $wpdb->prepare( "SELECT * FROM {$table_name} WHERE id = %d AND item_type = %s LIMIT 1;", array(
			$this->id,
			'cf_image',
		) );
	...
	public function optimize( $optimization_level = '' ) {
		...
		if ( is_wp_error( $is_image_backuped ) ) {
			...
			WIO_Logger::error( sprintf( 'Failed to backup with message: %s. Skipping optimization of custom folder', $is_image_backuped->get_error_message() ) );

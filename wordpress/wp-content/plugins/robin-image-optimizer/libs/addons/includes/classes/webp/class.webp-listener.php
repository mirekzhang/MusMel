...
/**
 * Class WRIO_Webp_Listener listens to new events via hooks.
 *
...
class WRIO_Webp_Listener {
	...
	public function init() {
		if ( WRIO_Webp_Collection::is_webp_enabled() ) {
			...
			add_action( 'wbcr/rio/attachment_restored', array( $this, 'process_attachment_restore' ) );
			add_action( 'wbcr/rio/cf_image_restored', array( $this, 'process_attachment_restore' ) );
			add_action( 'wbcr/rio/nextgen_image_restored', array( $this, 'process_attachment_restore' ) );
		}
	}
	...
	public function process_attachment_restore( $model ) {
		$item_params = array(
			...
			'item_type' => WRIO_Webp_Listener::DEFAULT_TYPE,
		);
		...
		foreach ( $delete_items as $item ) {
			...
			if ( empty( $extra ) ) {
				WIO_Logger::warning( sprintf( 'Failed to clean-up queue item #%s as it is missing extra data', $item->get_id() ) );
				continue;
			}
			...
			if ( ! empty( $converted_path ) ) {
				if ( @unlink( $converted_path ) ) {
					WIO_Logger::info( sprintf( 'Unlinked %s from disk, ready to delete item #%s from DB', $converted_path, $item->get_id() ) );
				} else {
					WIO_Logger::error( sprintf( 'Failed to unlink %s from disk', $converted_path ) );
				}
			}

			if ( $item->delete() ) {
				WIO_Logger::info( sprintf( 'Deleted #%s as attachment #%s was recovered', $item->get_id(), $item->get_object_id() ) );
			} else {
				WIO_Logger::error( sprintf( 'Failed to delete queue item #%s as delete() method failed', $item->get_id() ) );
			}
		}
	...
	public function process_queue_item( $model ) {
		if ( ! ( $model instanceof RIO_Process_Queue ) ) {
			WIO_Logger::info( 'Model must be instance of RIO_Process_Queue to be process by %s' . __FUNCTION__ );

			return false;
		}

		if ( ! $model->is_optimized() ) {
			WIO_Logger::info( sprintf( 'Skipping to process attachment #%s as it is not optimized', $model->get_id() ) );
	...
	public function process_attachment( $model ) {
		...
		WIO_Logger::info( sprintf( 'Ready to process attachment item #%s', $model->get_id() ) );

		$attachment = get_post( $model->get_object_id() );

		if ( empty( $attachment ) ) {
			WIO_Logger::warning( sprintf( 'No attachment found by #%s', $model->get_object_id() ) );
		...
		if ( ! in_array( $attachment->post_mime_type, $allowed_mimes ) ) {
			WIO_Logger::warning( sprintf( 'Attachment #%s with MIME type %s cannot be processed as only these are allowed: %s', $attachment->ID, $attachment->post_mime_type, implode( ', ', $allowed_mimes ) ) );
		...
		if ( empty( $attachment_meta ) ) {
			WIO_Logger::warning( sprintf( 'Unable to get attachment #%s meta such as height, abs. path, URL, etc. Skipping WebP processing...', $attachment->ID ) );
		...
		foreach ( $attachment_meta as $hash => $data ) {
			...
			WIO_Logger::info( sprintf( 'Ready to save hash "%s" (extra data: %s) as it does not exist yet', $hash, json_encode( $data ) ) );
			...
			$extra_data = new RIOP_WebP_Extra_Data( array(
	...
	public function process_custom_folder( $model ) {
		...
		WIO_Logger::info( sprintf( 'Ready to process custom folder item #%s', $model->get_id() ) );
	...
	public function process_nextgen( $model ) {
		...
		WIO_Logger::info( sprintf( 'Ready to process NextGen plugin item #%s', $model->get_id() ) );
		...
		$original_saved = $this->save( array(
			'item_hash'          => $model_extra_data->get_image_url(),
			'original_mime_type' => $model->get_original_mime_type(),
			'object_id'          => $model->get_object_id(),
		), $extra_data );
		...
		// Thumbnail
		$extra_data_thumbnail = new RIOP_WebP_Extra_Data( array(
			'convert_from'        => 'nextgen',
			'converted_from_size' => null,
		...
		$thumbmail_saved = $this->save( array(
	...
	public static function get_attachment_data( $attachment ) {
		...
		if ( empty( $attachment_meta ) ) {
			...
			if ( isset( $exploded_url[1] ) ) {
				$exploded_path_from_url = wp_normalize_path( trailingslashit( $dirs['basedir'] ) . trim( $exploded_url[1] ) );
				...
				$attachment_meta = wp_generate_attachment_metadata( $attachment->ID, $exploded_path_from_url );
	...
	public function save( $props, $extra_data ) {
		...
		if ( $is_saved ) {
			WIO_Logger::info( sprintf( 'Saved item with src "%s" and hash "%s" successfully', $extra_data->get_source_src(), $model->get_item_hash() ) );

			return $model;
		}

		WIO_Logger::info( sprintf( 'Failed to save item with src "%s" and hash "%s" as save() method failed, check SQL for errors', $extra_data->get_source_src(), $model->get_item_hash() ) );

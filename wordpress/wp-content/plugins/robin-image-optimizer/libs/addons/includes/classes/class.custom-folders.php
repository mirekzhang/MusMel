...
class WRIO_Custom_Folders {
	...
	public function optimizeImage( $image_id, $level = '' ) {
		...
		if ( $cf_image->isOptimized() ) {
			 ...
			 *
			 */
			do_action( 'wbcr/rio/cf_image_restored', $optimization_data );
		}
	...
	protected function deferredOptimizeImage( $image_id ) {
		...
		if ( ! $image_processor->isDeferred() ) {
			...
			WIO_Logger::error( sprintf( 'Server %s does not support deferred optimization', get_class( $image_processor ) ) );
	...
	public function processUnoptimizedImages( $max_process_per_request = 5 ) {
		...
		$responce = [
			...
			'last_optimized'  => $image_statistics->getLastOptimizationLogs( $max_process_per_request ),
			...
			foreach ( $optimized_images as $row ) {
				...
				if ( $cf_image->isOptimized() ) {
					...
					if ( ! is_wp_error( $restored ) ) {
						...
						WIO_Logger::error( sprintf( 'Failed to restore custom folder. Object info: %s', wp_json_encode( $cf_image ) ) );
	...
	public function webpSuccess( $queue_model ) {
		if ( ! class_exists( 'WRIO_Webp_Listener' ) ) {
			return false; // если не установлена премиум версия, то WebP не активен
		}
		if ( $queue_model->get_item_type() !== WRIO_Webp_Listener::DEFAULT_TYPE ) {
			return false;
		}

...
class WRIOP_Backup extends WIO_Backup {
	...
	public function backupNextgen( $nextgen_image ) {
		...
		if ( is_file( $original_thumbnail_file ) ) {
			if ( @copy( $original_thumbnail_file, $backup_thumbnail_file ) ) {
				WIO_Logger::error( sprintf( 'Failed to swap thumbnail file %s with %s as copy() failed', $backup_thumbnail_file, $original_thumbnail_file ) );
	...
	public function restoreAllNextGen( $limit = 50 ) {
		...
		foreach ( $nextgen_images as $nextgen_image ) {
			...
			if ( ! is_wp_error( $restored ) ) {
				...
				WIO_Logger::error( sprintf( 'Failed to restore nextgen image ID: %s as %s::%s failed with message: %s', $nextgen_image->object_id, get_class( 'WRIO_Image_Nextgen' ), 'restore()', $restored->get_error_message() ) );
			}
		}
		...
		if ( $nextgen_image_remane === null ) {
			WIO_Logger::error( sprintf( 'Failed to get remained number of nextgen image by SQL: %s', $nextgen_sql_remane ) );
		}
	...
	public function restoreAllCustomFolders( $limit = 50 ) {
		...
		foreach ( $cf_images as $cf_image ) {
			...
			if ( ! is_wp_error( $restored ) ) {
				...
				WIO_Logger::error( sprintf( 'Failed to restore Custom Folder ID: %s as %s::%s failed with message: %s', $cf_image->object_id, get_class( 'WRIO_Folder_Image' ), 'restore()', $restored->get_error_message() ) );
			}
		}
		...
		if ( $cf_image_remane === null ) {
			WIO_Logger::error( sprintf( 'Failed to get remained number of Custom Folder by SQL: %s', $cf_sql_remane ) );
		}
	...
	public function restoreNextgen( $nextgen_image ) {
		...
		if ( ! is_file( $backup_file ) ) {
			...
			WIO_Logger::error( sprintf( '%s, Nextgen image id: %s', $error_msg, $nextgen_image->get( 'id' ) ) );

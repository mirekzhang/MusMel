...
if (!function_exists('sin_setup')) {
  function sin_setup() {
    /*
     * Make theme available for translation.
     * Translations can be filed in the /languages/ directory.
     */
    load_theme_textdomain('sin', get_template_directory().'/languages');
    ...
    register_nav_menus( array(
      'menu-1' => esc_html__('Main menu', 'sin'),
      'menu-2' => esc_html__('Footer menu', 'sin')
    ));
    ...
    add_theme_support('custom-background', apply_filters('sin_custom_background_args', array(
...
add_action('after_setup_theme', 'sin_setup');
 ...
 */
function sin_content_width() {
  $GLOBALS['content_width'] = apply_filters('sin_content_width', 750);
}
add_action('after_setup_theme', 'sin_content_width', 0);

/**
 * Enqueue styles.
 */
if (!function_exists('sin_styles')) {
  function sin_styles() {  

    // google fonts
    wp_enqueue_style('google-fonts', '//fonts.googleapis.com/css?family=Open+Sans:400,400italic,700,700italic', array(), '1.0.33');
    ...
    wp_enqueue_style('ie10-viewport-bug-workaround', get_template_directory_uri().'/assets/css/ie10-viewport-bug-workaround.css', array(), '1.0.33');

    // font awesome
    wp_enqueue_style('font-awesome', get_template_directory_uri().'/assets/font-awesome/css/font-awesome.min.css', array(), '4.7.0');
    
    // main stylesheet
    wp_enqueue_style('sin-style', get_stylesheet_directory_uri().'/style.css', array(), '1.0.33');
  }
}
add_action('wp_enqueue_scripts', 'sin_styles');

/**
 * Enqueue scripts.
 */
if (!function_exists('sin_scripts')) {
  function sin_scripts() {
    ...
    wp_enqueue_script('ie10-viewport-bug-workaround', get_template_directory_uri().'/assets/js/ie10-viewport-bug-workaround.js', array(), '1.0.33', true);

    // theme js
    wp_enqueue_script('sin-theme', get_template_directory_uri().'/assets/js/theme.js', array('jquery'), '1.0.33', true);

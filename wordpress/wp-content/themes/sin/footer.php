    ...
    <div class="container">
      <div class="site-info clearfix">
        ...
        <div class="site-info-copyright <?php if (($show_social && !$social_url_empty) || ($show_menu && has_nav_menu('menu-2'))){?>have-site-info-nav<?php }?>">
          <a href="<?php echo esc_url( __( 'https://wordpress.org/', 'sin' ) ); ?>"><?php
            printf( esc_html__( 'Proudly powered by %s', 'sin' ), 'WordPress' );
          ?></a>
          <span class="sep"> | </span>
          <?php
            printf( esc_html__( 'Theme: %1$s by %2$s.', 'sin' ), 'Sin', '<a href="http://vsfish.com/">vsFish</a>' );
          ?>
        </div>
      </div><!-- .site-info -->
    </div>

...
function my_action(){
$conn = mysqli_connect("localhost", "root1", "123", "muslim_db");
if ($conn-> connect_error) {
  die("Connection failed:" . $conn-> connect_error);
	...
	if (isset($_POST['suburb'])) {
		$suburb = $_POST['suburb'];
	$sql = "select * FROM suburb_detail where suburb = '$suburb'";
}
else{
	$sql = "select * from suburb_detail where suburb = 'melbourne'";
}

$result_suburb = $conn-> query($sql);
 if($result_suburb->num_rows>0){
 	 while($row=$result_suburb-> fetch_assoc()) {
 	 	 $suburb_number = $row['suburb_number'];
 	 	if($row['1_bed_flat'] == 0){
			 $row['1_bed_flat']="Data unavailable";
		 }
		 else{
		 	$row['1_bed_flat']= "$".$row['1_bed_flat'];
		 }
		 if($row['2_bed_flat'] == 0){
			 $row['2_bed_flat']="Data unavailable";
		 }
		 else{
		 	$row['2_bed_flat']= "$".$row['2_bed_flat'];
		 }
		 if($row['3_bed_flat'] == 0){
			 $row['3_bed_flat']="Data unavailable";
		 }
		 else{
		 	$row['3_bed_flat']= "$".$row['3_bed_flat'];
		 }
		 if($row['2_bed_house'] == 0){
			 $row['2_bed_house']="Data unavailable";
		 }
		 else{
		 	$row['2_bed_house']= "$".$row['2_bed_house'];
		 }
		 if($row['3_bed_house'] == 0){
			 $row['3_bed_house']="Data unavailable";
		 }
		 else{
		 	$row['3_bed_house']= "$".$row['3_bed_house'];
		 }
		 if($row['4_bed_house'] == 0){
			 $row['4_bed_house']="Data unavailable";
		 }
		 else{
		 	$row['4_bed_house']= "$".$row['4_bed_house'];
		 }
		 if($row['no_of_school'] == 0){
			 $row['no_of_school']="No schools in this area";
		 }
		 else{
		 	$row['no_of_school'] = $row['no_of_school'] . " <a href='http://52.63.236.114/index.php/restaurant?suburb_number= ".$suburb_number."'>Details</a>";
		 }
		 if($row['no_of_mosque'] == 0){
			 $row['no_of_mosque']="No Mosques in this area";
		 }
		 else{
		 	$row['no_of_mosque'] = $row['no_of_mosque'] . " <a href='http://52.63.236.114/index.php/restaurant?suburb_number= ".$suburb_number."'>Details</a>";
		 }
		 if($row['no_of_food'] == 0){
			 $row['no_of_food']="No Halal Restaurants in this area";
		 }
		 else{
		 	$row['no_of_food'] = $row['no_of_food'] . " <a href='http://52.63.236.114/index.php/restaurant?suburb_number= ".$suburb_number."'>Details</a>";
		 }
...
add_action('wp_ajax_new_action','new_action');
add_action('wp_ajax_nopriv_new_action','new_action');
function new_action(){
	$response = "data1";
